...
| `templates/progress.md` | Creator-only | 生成最小项目状态面 |
| `templates/session-handoff.md` | Creator-only | 仅在复杂度需要时生成 |
...
持续状态能力是必需的，但不强制两个固定文件：

- `progress.md` 是默认轻量状态面；
- `session-handoff.md` 仅用于跨会话、多 agent 或复杂交接；
- 等价的项目工件可以满足能力要求；
